...
function navigate(params: Record<string, string | number | null | undefined>, partial = false) {
    ...
    router.get(baseUrl(), clean, partial ? { only: ['tableData', 'cards'] } : {});
}
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-5 p-4 xl:p-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <a
                :href="excelUrl"
                :class="tableData?.error ? 'opacity-40 pointer-events-none' : 'hover:bg-emerald-700'"
        ...
        <Deferred data="tableData">
            <template #fallback>
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="h-9 w-72 animate-pulse rounded-lg bg-gray-200 dark:bg-gray-700" />
                    ...
                    <div class="overflow-auto max-h-[60svh]">
                        <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
                            <thead class="sticky top-0 z-10 bg-gray-50 dark:bg-gray-700/50">
                                <tr>
                                    <th v-for="h in ['No', 'No. Registrasi', 'Tanggal', 'Jam', 'No. RM', 'Nama Pasien', 'J/K', 'Jaminan', 'Kategori', 'Unit', 'Lokasi', 'Dokter', 'Plafond', 'T. Mitra', 'T. Tunai', 'Total', 'Sisa']" :key="h" class="px-3 py-3 text-left font-semibold text-gray-700 dark:text-gray-300">{{ h }}</th>
                                </tr>
                            </thead>
                            <tbody class="animate-pulse divide-y divide-gray-200 dark:divide-gray-700">
                                <tr v-for="i in 8" :key="i">
                                    <td v-for="j in 17" :key="j" class="px-3 py-3">
                                        <div class="h-4 rounded bg-gray-200 dark:bg-gray-700" :class="j === 6 ? 'w-32' : j === 1 ? 'w-6' : 'w-16'" />
                                    </td>
                                </tr>
                            </tbody>
                        </table>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400">
                    {{ (tableData?.pagination.total ?? 0).toLocaleString('id-ID') }} record
                </span>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-if="!(tableData?.items?.length)">
                                <td colspan="17" class="px-4 py-10 text-center text-gray-400 dark:text-gray-500">
                                    {{ tableData?.error ? 'Tidak ada data — koneksi ke database TARAKAN gagal.' : 'Tidak ada data.' }}
                                </td>
                            </tr>
                            <tr
