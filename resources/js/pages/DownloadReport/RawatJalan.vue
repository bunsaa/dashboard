...
// ── Modal detail per-poli ──────────────────────────────────────────────────
const selectedPoli = ref<PoliEntry | null>(null);
function openDetail(row: PoliEntry) { selectedPoli.value = row; }
function closeDetail() { selectedPoli.value = null; }
        ...
        >
            <div class="w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-gray-800">
                ...
                <div class="flex items-start justify-between border-b border-gray-100 px-5 py-4 dark:border-gray-700">
                    <div>
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">Detail Kunjungan Rawat Jalan</h3>
                        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ selectedPoli.NamaPoli }}</p>
                    </div>
                    <button
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            >
                                ...
                                <td class="px-5 py-2.5 text-right tabular-nums text-gray-700 dark:text-gray-300">
                                    {{ (selectedPoli[`year_${y}`] as number ?? 0).toLocaleString('id-ID') }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="border-t border-gray-100 px-5 py-3 dark:border-gray-700">
                    <button
                        ...
                        @click="triggerDownload(poliExcelUrl(selectedPoli.NamaPoli))"
                    >
                        <Download :size="12" />
                        Download Excel
                    </button>
