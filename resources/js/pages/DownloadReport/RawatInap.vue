...
defineOptions({
    layout: (props: { currentTeam?: Team | null }) => ({
        breadcrumbs: [
            ...
            {
                title: 'Rawat Inap',
                href: props.currentTeam ? downloadReportRoutes.rawatInap(props.currentTeam.slug).url : '/',
            },
        ],
    }),
});
...
function applyYearFilter() {
    ...
    router.get(
        downloadReportRoutes.rawatInap(currentTeam.value.slug).url,
...
const excelAllUrl = computed(() => {
    ...
    return rawatInapRoutes.excel(currentTeam.value.slug, {
        query: { fromYear: props.fromYear, ...(q ? { q } : {}) },
    }).url;
});

function ruanganExcelUrl(namaRuangan: string): string {
    if (!currentTeam.value) return '#';
    return rawatInapRoutes.excel(currentTeam.value.slug, {
        query: { fromYear: props.fromYear, poli: namaRuangan },
    }).url;
}
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 xl:p-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h1 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Laporan Kunjungan Pasien Rawat Inap</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    Data kunjungan rawat inap RSUD Tarakan tahun {{ props.fromYear }}–{{ props.tahun }}
                </p>
            </div>
            <button
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <!-- Ruangan search -->
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Cari Ruangan:</label>
                <input
                    v-model="filterInput"
                    list="ruangan-datalist-ri"
                    type="text"
                    placeholder="Ketik sebagian nama ruangan…"
                ...
                />
                <datalist id="ruangan-datalist-ri">
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400">
                {{ filteredRekap.length }} dari {{ rekapByPoli.length }} ruangan
            </span>
        </div>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <template v-else>
                ...
                >
                    <span class="text-xs text-gray-500 dark:text-gray-400">
                        Hal {{ currentPage }} / {{ totalPages }} &nbsp;·&nbsp; {{ filteredRekap.length }} ruangan
                    </span>
